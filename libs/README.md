...
以上是默认编译版本，可在 `gradle.properties` 调整，或通过命令行覆盖，例如 `gradlew.bat build -Pepicfight_version=版本号`。本目录需放置与所选版本对应的 JAR。

运行时不要求与编译版本完全一致：Epic Fight 允许 20.14.17 及以上，Avalon 允许 20.12.6.5 及以上，Photon 允许 1.1.17 及以上，Photon/Epic Fight 联动允许 1.1.4 及以上，均不设置上限。Minecraft 仍为 1.20.1、Forge 为 47 及以上。放宽加载范围不代表已验证所有新版 API；目前构建验证使用上述默认依赖。其他前置自身声明的依赖范围仍需满足。
